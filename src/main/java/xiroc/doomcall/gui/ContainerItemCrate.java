...
public class ContainerItemCrate extends Container {
	...
	public ContainerItemCrate(IInventory playerInv, TileEntityItemCrate crate) {
		int i = -18;
		int j;
		int k;
		...
		for (j = 0; j < 3; ++j) {
			for (k = 0; k < 5; ++k) {
				addSlotToContainer(new Slot(crate, index++, 44 + k * 18, 17 + j * 18));
			}
		}
